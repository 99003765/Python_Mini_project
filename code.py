...
class Find_Word:                   # Creating Class
    ...
    def pattern_search(self,):             # Creating methods for pattern search
        ...
        match = re.findall(self.pattern, self.f, re.M | re.I)          # Searching given pattern in the input file and ignoring alphabatic cases
        extension = ".txt"
        file_name = str(self.pattern) + extension                     # Converting pattern given to string and adding txt extension
        f_out = open(file_name, "w+")                                 # Creating and opening output file with Read and write mode
        f_out.write("The count of words:" + str(len(match)))          # Displaying word count         
        f_out.write(str("\n\n"))
        split_f = self.f.split()                                      # Splitting up of sentences to words of input file
        f = open(file_name, "a+")
        for i in range(len(split_f)):                                 # opening input file in apend, write,read mdoe
            match = re.match(self.pattern, split_f[i], re.M | re.I)   # matching the pattern to splitted words
            if match:
                count += 1                                            
                String = (split_f[i-1]+" "+split_f[i]+" "+split_f[i+1]+"\n")      # creating the matched string including its previous and next words
